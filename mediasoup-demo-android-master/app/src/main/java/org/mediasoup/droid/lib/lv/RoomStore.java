...
public class RoomStore {
  ...
  public void setRoomActiveSpeaker(String peerId, int volume) {
    roomInfo.postValue(roomInfo -> roomInfo.setActiveSpeakerId(peerId, volume));
  }

  public void clearRoomActiveSpeaker(){
    roomInfo.postValue(RoomInfo::clearActiveSpeakers);
  }
    ...
    roomInfo.postValue(
        roomInfo -> {
          if (!TextUtils.isEmpty(peerId) && peerId.equals(roomInfo.getActiveSpeakerId())) {
            //roomInfo.setActiveSpeakerId(null, -127);
            roomInfo.clearActiveSpeakers();
          }
